          ...
          <div className="col-6 d-flex flex-column h-100">
            <div className="shining-border-wrapper">
              <Card className="fullscreen-gameplay-card" style={{ backgroundColor: 'rgba(255, 255, 255, 0.4)', backdropFilter: 'blur(10px)' }}>
                {/* Added overflow-hidden and adjusted padding to keep everything tightly bound inside */}
                <Card.Body className="d-flex flex-column align-items-center justify-content-between p-3 overflow-hidden">
                  <Card.Title className="fs-4 fw-bold text-dark mb-2 text-center flex-shrink-0">Scan to join!</Card.Title>
                  
                  {/* Constrained the QR box using aspect-ratio and flex-shrink so it never cuts off the text below */}
                  <div className="p-2 bg-white rounded shadow-sm d-flex justify-content-center align-items-center border w-50 flex-grow-1" style={{ minHeight: 0, aspectRatio: '1/1' }}>
                    <QRCodeSVG 
                      value={joinUrl} 
                  ...
                  {/* Room code is now guaranteed space to render */}
                  <div className="mt-2 fs-2 fw-bold flex-shrink-0" style={{ color: '#014eb6', letterSpacing: '0.15em' }}>
                    {roomCode}
                  </div>
                </Card.Body>
              </Card>
